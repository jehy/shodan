...
function createHeader(duty) {
  return [{
    ...
    text: {
      type: 'mrkdwn',
      text: `*Статистика.* Уважаемые дежурные ${duty[0]}, ${duty[1]}, у нас что-то горит...`,
    },
  }];
}
function createMessage(messages, duty) {
  return createHeader(duty).concat(messages.map((el, i) => {
    ...
    return {
      ...
      text: {
        ...
        text: `*${i + 1}.* 
           ...
           рост: ${prevInterval}
           ${filters[el.typeErr] ? ` ${filters[el.typeErr].description}` : ''}
...
async function writeToDb(elements) {
  ...
  const toSlackMess = await Promise.reduce(elements, async (acc, cur) => {
    await knex
      ...
      .first('id', 'added')
      .then(async (value) => {
        if (!value || (value && isExpiredTime(value.added))) {
          log.info(`Write errors to database ${JSON.stringify(cur)}`);
          await knex.insert({msgName: cur.msgName, typeErr: cur.typeErr}).into(slackCfg.table);
          acc.push(cur);
        }
      });
    return acc;
