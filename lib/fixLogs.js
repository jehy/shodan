...
function fixLogEntry(logEntry) {
  ...
  let messageName = logEntry._source.msgName; // by default, taken from root
  if (!messageName && logEntry._source.data) {
    if (logEntry._source.data.msgName) {
      messageName = logEntry._source.data.msgName; // if not from root then from data
    } else if (logEntry._source.data.event) {
